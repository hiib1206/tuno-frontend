...
import { ArrowUpRight, BarChart3, Loader2, Search, Star } from "lucide-react";
import { useRouter } from "next/navigation";
import { useCallback, useEffect, useState } from "react";

type MainTab = "stock" | "portfolio";
type ListTab = "search" | "watchlist";
...
export default function QuantAnalysisPage() {
  ...
  useEffect(() => {
    ...
    // 약간의 딜레이 후 fade-in (더 자연스러운 효과)
    requestAnimationFrame(() => setIsGreetingReady(true));
  }, []);
  ...
  const [selectedStock, setSelectedStock] = useState<{
    ...
    return latest ? { code: latest.code, exchange: latest.exchange } : null;
  });
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (candleRes.success && candleRes.data && candleRes.data.candles.length > 0) {
          const chartData = candleRes.data.candles.map((c) => ({
    ...
    <div className="h-full flex items-center justify-center">
      <div className="w-full max-w-3xl">
        ...
        <div className="flex items-center justify-center gap-3 mb-4">
          ...
          <h1
            className={cn(
              "text-3xl font-medium text-foreground transition-opacity duration-300",
              isGreetingReady ? "opacity-100" : "opacity-0"
            )}
          >
            {greetingMessage}
          </h1>
        ...
        {/* 콘텐츠 */}
        {mainTab === "stock" && (
          <div className="bg-background-1 rounded-lg overflow-hidden h-[500px]">
            <div className="flex h-full">
              {/* 왼쪽: 검색/관심종목 탭 */}
              <div className="w-72 border-r border-border-2 flex flex-col">
                {/* 리스트 탭 */}
