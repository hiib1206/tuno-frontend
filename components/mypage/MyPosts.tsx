...
export default function MyPosts() {
  ...
  // 삭제 실패한 게시글 ID와 메시지를 관리하는 state
  ...
  useEffect(() => {
    ...
    fetchPosts();
  }, [isAuthLoading, currentPage, sortOption]);
  ...
  // 정렬 변경 핸들러
  const handleSortChange = (option: PostSortOption) => {
  ...
  const executeDelete = async () => {
    try {
      ...
      const response = await postApi.deletePosts(idsArray);
      ...
      if (response.success) {
        ...
        const refreshResponse = await postApi.getMyPosts({
          page: currentPage,
          limit: MYPOSTS_LIST_LIMIT,
          sort: sortOption.sort,
          order: sortOption.order,
        ...
        if (refreshResponse.success) {
          ...
          const maxPage = Math.max(
            1,
            Math.ceil(newTotalCount / MYPOSTS_LIST_LIMIT)
          );

          // 현재 페이지에 게시글이 없고, 이전 페이지가 있으면 이전 페이지로 이동
          if (newPosts.length === 0 && currentPage > 1) {
            setCurrentPage(currentPage - 1);
            // useEffect가 currentPage 변경을 감지하여 fetchPosts() 호출
        ...
        if (failedCount > 0) {
          ...
          setFailedPostIds(new Map());
          toast({
            title: `${successCount}개의 게시글이 삭제되었습니다.`,
          });
        }
  ...
  return (
    <div className="flex-1 rounded-lg bg-background-1 py-4 px-4 md:px-6">
      ...
      {loading ? (
        ...
        <div className="py-48">
          <ErrorState message={error ?? "게시글을 불러오는데 실패했습니다."} />
        </div>
      ) : (
        <div className="min-h-[600px] mt-2">
          {/* 정렬 및 삭제 버튼 영역 */}
                  ...
                  {POST_SORT_OPTIONS.map((option) => (
                    <DropdownMenuItem
                      ...
                      className={cn(
                        ...
                        sortOption.value === option.value && "text-accent-text"
                      )}
                    >
                      {option.label}
                    </DropdownMenuItem>
      ...
      <ConfirmDialog
        ...
        title="게시글 삭제"
        description={`정말로 ${selectedIds.size}개의 게시글을 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.`}
